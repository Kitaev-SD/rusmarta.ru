...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $pagerParameters)))
{
	...
	//====================================
	$subFilter = array_merge($arrFilter, $arFilter);
	if ($subFilter['SECTION_ID'] > 0){
        $sectionFilter = Array ('ACTIVE' => 'Y', 'GLOBAL_ACTIVE' => 'Y', 'IBLOCK_ID' => $arParams["IBLOCK_ID"], 'UF_SECTIONS' => $subFilter['SECTION_ID']);        
        $rsSections = CIBlockSection::GetList([], $sectionFilter, false, ['ID']);
        while($arSection = $rsSections->GetNext()){
            $subFilter['SECTION_ID'] = $arSection['ID'];
            $rsElements = CIBlockElement::GetList($arSort, $subFilter, false, $arNavParams, $arSelect);
            while($arItem = $rsElements->GetNext()){
                $items[] = $arItem;
            }
        }
