...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	//====================================================
	//ARGIT
	if(!isset($sectionFilter['UF_BOTTOM_TAGS']))	
        $sectionFilter['UF_BOTTOM_TAGS'] = 0;
	...
	while($arSection = $rsSections->GetNext())
        $SECTIONS[] = $arSection;
    if ($arParams["SECTION_ID"] > 0){
        $arFilter = Array ('ACTIVE' => 'Y', 'GLOBAL_ACTIVE' => 'Y', 'IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CNT_ACTIVE' => 'Y', 'UF_SECTIONS' => $arParams["SECTION_ID"]);        
        $arFilter['UF_BOTTOM_TAGS'] = $sectionFilter['UF_BOTTOM_TAGS'];        
