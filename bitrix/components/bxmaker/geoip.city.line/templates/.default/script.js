...
if (!!window.BXmakerGeoIPCityLineConstructor === false) {
	...
	BXmakerGeoIPCityLineConstructor.prototype.showCity = function (city) {
		...
		$.ajax({
			url: '/bitrix/components/bxmaker/geoip.city.line/templates/.default/ajax.php',
			type: 'get',
			cache: false,
			data: 'city='+city,
		}).done(function(data){
			$('#telephone-html').html(data);
		}).complete(function(){
		...
		$.ajax({
			url: '/bitrix/components/bxmaker/geoip.city.line/templates/.default/ajax_delivery.php',
			type: 'get',
			cache: false,
			data: 'city='+city,
		}).done(function(data){
			$('#header_location').html(data);
		}).complete(function(){
