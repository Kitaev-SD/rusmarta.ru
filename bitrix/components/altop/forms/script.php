		...
		if(!empty($post)) {
			if($arProp["PROPERTY_TYPE"] == "S") {
				...
				foreach($post as $arFile) {
					...
					$arProps[$arProp["CODE"]][] = CIBlock::makeFileArray($arFile);
				}
				unset($arFile);
			}
		}
...
if($elementId = $el->Add($arFields)) {
	...
	if (!empty($_REQUEST['PHONE'])) {
        $roistatData = array(
            ...
            'key' => 'N2VlNWVmMmMwNWVkNWE2ZDg3ZTU1NDhiZmIyODYxMjA6NjcxMjQ',
            'title' => 'Новый лид с формы "Под заказ"',
            'comment' => 'Комментарий: ' . $_REQUEST['MESSAGE'] . PHP_EOL . 'Время звонка: ' . $_REQUEST['TIME'],
            'name' => $_REQUEST['NAME'],
