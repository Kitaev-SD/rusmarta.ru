...
<?
	...
	<div class="edost_compact_hide">
		<div class="edost_button_big <?=($active ? 'edost_button_head' : 'edost_button_big_red edost_button_big_active')?>" onclick="edost_window.set('paysystem', 'head=<?=GetMessage('SOA_TEMPL_PAYSYSTEM_HEAD')?>')">
...
<?
	if ($compact != '') $w = '';
	else $w = 'max-width: '.$table_width.'px;';
?>
	<div id="edost_paysystem_div" class="edost_div edost_compact_div edost_paysystem_div" style="<?=$w?>">
<?
...
<?	}
	...
	if (empty($arResult['ORDER_TOTAL_LEFT_TO_PAY_FORMATED']) || $arResult['ORDER_TOTAL_LEFT_TO_PAY'] > 0) {
		...
		foreach($arResult['PAY_SYSTEM'] as $v) {
			...
			$onclick_get = "window.edost_window.submit('".$id."')";
...
<?					if ($disable) { ?>
					<td class="edost_resize_tariff_show edost_resize_button edost_button_cod_disable" align="right">
						<div class="edost_button_cod_disable" onclick="edost_window.set('tariff', 'head=<?=GetMessage('SOA_TEMPL_WINDOW_COD_HEAD')?>;class=edost_compact_main edost edost_compact_cod_main;cod_id=<?=$id?>')">
...
<?								if (empty($v['edost_cod']) && $compact != '') { ?>
						<div class="edost_button_big2" onclick="edost_window.set('paysystem', 'head=<?=GetMessage('SOA_TEMPL_PAYSYSTEM_HEAD_PREPAY')?>;class=edost_window_prepay')">
...
<?				if ($disable) { ?>
				<tr class="edost_resize_button2">
					<td colspan="5" class="edost_resize_show edost_button_cod_disable edost_button edost_compact_window_hide edost_supercompact_hide" align="right" style="padding-top: 5px;">
						<div class="edost_button_cod_disable edost_change_button" style="width: auto; max-width: 280px;" onclick="edost_window.set('tariff', 'head=<?=GetMessage('SOA_TEMPL_WINDOW_COD_HEAD')?>;class=edost_compact_main edost edost_compact_cod_main;cod_id=<?=$id?>')">
							<span><?=GetMessage('SOA_TEMPL_COD_DISABLE_BUTTON')?></span>
						</div>
					</td>
				</tr>
...
<?				if (empty($v['edost_cod']) && $compact != '' && !empty($format['prepay_change'])) { ?>
				<tr class="edost_resize_button2">
					<td colspan="5" class="edost_resize_show edost_button edost_compact_window_hide edost_supercompact_hide" align="right" style="padding-top: 5px;<?=(1==2 && !$checked ? ' opacity: 0.5;' : '')?>">
						<div class="edost_button_big2 edost_change_button" onclick="edost_window.set('paysystem', 'head=<?=GetMessage('SOA_TEMPL_PAYSYSTEM_HEAD_PREPAY')?>;class=edost_window_prepay')">
							<span><?=GetMessage('SOA_TEMPL_PREPAY_CHANGE_BUTTON')?></span>
						</div>
					</td>
				</tr>
