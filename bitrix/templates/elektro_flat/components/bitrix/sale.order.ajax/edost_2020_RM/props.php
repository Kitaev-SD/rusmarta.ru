...
?>
	<script type="text/javascript">
		function edost_SetTemplateLocation(param) {

			if (param === 'back') {
				var E = document.getElementById('edost_location_city_template_div');
				if (E) E.innerHTML = '';

				var E = document.getElementById('edost_location_div');
				edost_resize.change_class(E, ['edost_location_button_hide', ''], 1);
				...
				loadingForm();
				var E = document.getElementById('edost_location_city_template_div');
				if (E) E.innerHTML = '<div class="edost_delivery_loading"><?=GetMessage('SOA_TEMPL_LOADING')?></div>';
				...
				var ar = document.getElementsByClassName('edost_city_link');
				if (ar && ar[0]) ar[0].click();
				...
				var E = document.getElementById('edost_location_div');
				edost_resize.change_class(E, ['edost_location_button_hide', ''], 0);
...
if (!function_exists('DrawInput')) {
	function DrawInput($v, $fast = false, $submit = false) {
...
?>
		...
		<input type="<?=($type == 'tel' ? $type : 'text')?>" <?=(in_array($type, array('email', 'tel', 'name', 'organization')) ? 'autocomplete="'.$type.'"' : '')?> autocorrect="off" spellcheck="false" <?=($type == 'email' ? 'autocapitalize="off"' : '')?> maxlength="250" size="<?=$v['SIZE1']?>" value="<?=$v['VALUE']?>" <?=($fast ? 'data-name="'.$v['FIELD_NAME'].'"' : 'name="'.$v['FIELD_NAME'].'" id="'.$v['FIELD_NAME'].'"')?> data-type="<?=$type?>" oninput="window.edost_window.input('update', this, event)" <?=($submit ? 'onchange="submitForm()"' : '')?> <?=($fast ? 'onfocus="window.edost_window.input(\'focus\', this)" onblur="window.edost_window.input(\'blur\', this)"' : '')?>>
<?
	}
}
				...
				if (count($arResult['PERSON_TYPE']) > 1) {
					foreach($arResult['PERSON_TYPE'] as $v) { ?>
						<div class="edost_person_type<?=($v['CHECKED'] == 'Y' ? ' edost_person_type_active' : '')?>" onclick="this.children[0].checked = true; submitForm();">
							<input type="radio" id="PERSON_TYPE_<?=$v['ID']?>" name="PERSON_TYPE" value="<?=$v['ID']?>"<?=($v['CHECKED'] == 'Y' ? ' checked="checked"' : '')?> onclick2="submitForm()"> <label for="PERSON_TYPE_<?=$v['ID']?>"><?=$v['NAME']?></label>
						</div>
<?					} ?>
...
<?				}
			...
			if ($f_key == 'comment') {
				...
				echo '<textarea name="ORDER_DESCRIPTION" id="ORDER_DESCRIPTION" style="width: 100%; max-width: 100%; min-height: 120px;">'.$arResult['USER_VALS']['ORDER_DESCRIPTION'].'</textarea>';
				DrawPropHead($mono);
			}
...
<?				}
					...
					DrawInput($v, false, $v['IS_ZIP'] == 'Y' && empty($arResult['edost']['locations_installed']) ? true : false);
...
<?  			}
					...
					<textarea rows="<?=$rows?>" cols="<?=$v['SIZE1']?>" name="<?=$v['FIELD_NAME']?>" id="<?=$v['FIELD_NAME']?>"><?=$v['VALUE']?></textarea>
<?				}
