...
<div <?=(!$total_compact ? 'id="order_total_inside"' : '')?> class="edost edost_main edost_template_div">
...
<?			$payed_from_account = (!empty($arResult['ORDER_TOTAL_LEFT_TO_PAY_FORMATED']) ? true : false);
			...
			<div class="edost_order_total_info <?=($payed_from_account ? 'edost_order_total_info2 edost_order_total_white' : 'edost_order_total_main')?>" style="margin-top: 0;">
				<span><?=GetMessage($payed_from_account ? 'SOA_TEMPL_SUM_IT_DISCOUNT' : 'SOA_TEMPL_SUM_IT')?></span>
				<span><?=$p?></span>
			</div>
<?			} ?>
...
<?			if (!empty($arResult['ORDER_TOTAL_LEFT_TO_PAY_FORMATED'])) {
			...
			<div class="edost_order_total_info edost_order_total_main">
				<span><?=GetMessage('SOA_TEMPL_SUM_IT')?></span>
				<span><?=$p?></span>
			</div>
<?			} ?>
...
<?		if ($delivery_location !== false && $save_button && $total_compact) { ?>
		<div id="order_save_button2" class="edost_button_big" onclick="if (window.edost_window.props()) submitForm('Y')"><span><?=GetMessage('SOA_TEMPL_BUTTON')?></span></div>
<?		} ?>
	</div>
...
<?	if ($total_compact && ($arParams['FAST'] == 'full' || $arParams['FAST'] == 'compact')) { ?>
	...
	<div class="edost_main2 edost_template_div2 edost_fast2" style="text-align2: center; padding: 20px;">
		<div class="edost_button_big2" style="margin: 0 auto; width: 94%; padding: 8px 0;" onclick="edost_Agreement('fast')"><span style="display: inline-block; font-size: 17px;"><?=GetMessage('SOA_TEMPL_FAST_BUTTON')?></span></div>
