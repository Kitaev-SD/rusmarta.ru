...
if (count($GLOBALS['productSlider1']) > 0){
    ...
    <div class="h3">Ïîõîæèå òîâàðû</div>
    <?
...
if (count($GLOBALS['productSlider2']) > 0){
    ...
    <div class="h3">Âàì ìîæåò ïîíðàâèòüñÿ</div>
    <?
...
//if(!empty($_REQUEST["sort"]) || !empty($_REQUEST["order"]) || !empty($_REQUEST["limit"]) || !empty($_REQUEST["view"]) || !empty($_REQUEST["action"]) || !empty($_REQUEST["set_filter"]) || !empty($_REQUEST["PAGEN_1"])):
	...
	$APPLICATION->SetPageProperty("title",str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' ���.', $APPLICATION->GetPageProperty("title")));
	$APPLICATION->SetPageProperty("description", str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' ���.', $APPLICATION->GetPageProperty("description")));
	$APPLICATION->SetPageProperty("keywords", str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' ���.', $APPLICATION->GetPageProperty("keywords")));
