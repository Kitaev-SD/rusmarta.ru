...
if(!empty($_REQUEST["PAGEN_1"]) && $_REQUEST["PAGEN_1"] > 1):
	...
	$APPLICATION->SetTitle($arCurSection["NAME"] . " ó ÒÚ‡ÌËˆ‡ " . $page);

	$APPLICATION->SetPageProperty("title", (!empty($arCurSection["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]) ? $arCurSection["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"] : $arCurSection["NAME"]) . " ó ÒÚ‡ÌËˆ‡ " . $page);
	$APPLICATION->SetPageProperty("keywords", "");
	$APPLICATION->SetPageProperty("description", $APPLICATION->GetProperty('description') . " —Ú‡ÌËˆ‡ " . $page . ".");
endif;
...
//CANONICAL//
	...
	$APPLICATION->SetPageProperty("title",str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' Û·.', $APPLICATION->GetPageProperty("title")));
	$APPLICATION->SetPageProperty("description", str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' Û·.', $APPLICATION->GetPageProperty("description")));
	$APPLICATION->SetPageProperty("keywords", str_replace('{#min_price}', (!empty($price["RESULT_PRICE"]["DISCOUNT_PRICE"])?$price["RESULT_PRICE"]["DISCOUNT_PRICE"]:$price["RESULT_PRICE"]["BASE_PRICE"]).' Û·.', $APPLICATION->GetPageProperty("keywords")));
