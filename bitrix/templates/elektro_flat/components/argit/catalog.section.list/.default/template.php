								...
								<span class="child">
									<span class="image">
										<?if(is_array($arChild["PICTURE"])):?>
											<img class="data-lazy-src" data-lazy-src="<?=$arChild['PICTURE']['SRC']?>" width="<?=$arChild['PICTURE']['WIDTH']?>" height="<?=$arChild['PICTURE']['HEIGHT']?>" title="<?=$arChild['NAME']?>" />
										<?else:?>
											<img class="data-lazy-src" data-lazy-src="<?=SITE_TEMPLATE_PATH?>/images/no-photo.jpg" width="50" height="50" taitle="<?=$arChild['NAME']?>" />
										<?endif;?>
									</span>
