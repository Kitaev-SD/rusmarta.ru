						...
						if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])) {
							...
							<div class="add2basket_block">
								<form action="<?=$curPage?>" class="add2basket_form">
									...
									<button type="button" id="<?=$arItemIDs['PROPS_BTN']?>" class="btn_buy" name="add2basket" onclick="ym(22745254,'reachGoal','addcart'); ga('send', 'event', 'addcart', 'click'); add2cart_gtag_al(<?=$arElement['ID']?>,<?=$arElement["MIN_PRICE"]["RATIO_PRICE"]?>);"><i class="fa fa-shopping-cart"></i><span><?=GetMessage("CATALOG_ELEMENT_ADD_TO_CART")?></span></button>
								</form>
							</div>
...
<script>
	function add2cart_gtag_al(id, price){
