...
/********************vp*********************/
if (explode('?', $_SERVER['REQUEST_URI'], 2)[0] != strtolower(explode('?', $_SERVER['REQUEST_URI'], 2)[0] )) { //Редирект url на нижний регистр
	if(!empty(explode('?', $_SERVER['REQUEST_URI'], 2)[1]))
		$b = '?'.explode('?', $_SERVER['REQUEST_URI'], 2)[1];
	header('Location: https://'.$_SERVER['HTTP_HOST'] . 
			strtolower(explode('?', $_SERVER['REQUEST_URI'], 2)[0]).$b, true, 301);
	exit();
}
...
<?/*vp*/?>
    ...
    <script  defer src="https://www.googletagmanager.com/gtag/js?id=UA-59688382-1"></script>
    <script defer>
...
//	}
	...
	<script>
/*
		let wrapperBlock = $('.center.outer'),
			wrapperBlockLeft = wrapperBlock.offset().left,
			modalBlockFirst = $('.modal_no_cookie_first'),
			modalBlockSecond = $('.modal_no_cookie_second'),
			checkCookie = document.cookie;

		if(!checkCookie) {

			if($(window).width() > '1014') {
				modalBlockFirst.css('right', wrapperBlockLeft + 'px');
				modalBlockFirst.fadeIn();
				modalBlockSecond.fadeIn();

			} else {
				modalBlockFirst.css('width', wrapperBlock.width() + 'px');
				modalBlockFirst.fadeIn();
			}

		}

		$('.modal_no_cookie_close').on('click', function(e) {
			let elem = $(e.target);
			elem.closest('.modal_no_cookie').fadeOut();
		});
*/
	</script>
</head>
