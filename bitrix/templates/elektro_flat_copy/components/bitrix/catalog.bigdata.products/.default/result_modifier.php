...
/*vp*/
    ...
    {
        if(!in_array($property["ID"], $arPid)&& !in_array($property["CODE"], $arPid)) {
            if($property["CODE"])$arResOther[$property["CODE"]] = CIBlockFormatProperties::GetDisplayValue($arElement, $arElement["PROPERTIES"][$property["CODE"]], "catalog_out_other");
            else
            $arResOther[$property["ID"]] = CIBlockFormatProperties::GetDisplayValue($arElement, $arElement["PROPERTIES"][$property["ID"]], "catalog_out_other");
        }
    }
    $arResult["ITEMS"][$key]["DISPLAY_PROPERTIES_OTHER"] = $arResOther;
/*vp*/		
