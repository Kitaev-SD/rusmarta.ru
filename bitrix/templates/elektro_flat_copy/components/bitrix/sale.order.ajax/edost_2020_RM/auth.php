...
<?		if($arResult['AUTH']['new_user_registration'] == 'Y') echo '<h4 style="text-align: center;">'.GetMessage('STOF_2REG').'</h4>'?>
...
<?	if ($arResult['AUTH']['new_user_registration'] == 'Y') {
	...
	<div class="edost edost_main edost_template_div">
<?		if($arResult['AUTH']['new_user_registration'] == 'Y') echo '<h4 style="text-align: center;">'.GetMessage('STOF_2NEW').'</h4>'?>
		<div class="edost_div" style="padding: 0 20px 20px 20px;">
...
<?			} ?>
			...
			<div style="padding-top: 15px; text-align: center;">
				<div class="edost_button_big" style="display: inline-block; padding: 10px; min-width: 250px;" onclick="if (window.edost_window.props()) BX('order_auth_new').submit()"><span style="font-size: 20px; line-height: 20px;"><?=GetMessage('STOF_NEW')?></span></div>
