...
<?			if (!$edost_catalogdelivery) { ?>
			...
			if (!E || E.value != 'edost:' + profile) <?=$calculate_function?>;
<?			} ?>
			return;
		}
...
<?		if (!$edost_catalogdelivery) { ?>
		...
		for (var i = 0; i < ar.length; i++) if (ar[i].id == 'ID_DELIVERY_edost_' + mode || ar[i].id == 'ID_DELIVERY_edost_' + mode + '_2') {
...
<?	if ($head_button) { ?>
	<div class="edost_window_hide edost_compact_hide">
		<div id="edost_get_delivery_button" class="edost_button_big <?=($active ? 'edost_button_head' : 'edost_button_big_red edost_button_big_active')?>" onclick="edost_window.set('<?=($supercompact_format !== false ? $supercompact_format : 'delivery')?>', 'head=<?=GetMessage('SOA_TEMPL_DELIVERY_HEAD')?>;class=<?=($supercompact_format !== false ? '' : 'edost_compact_main edost_compact_main2')?>')">
...
<?
		...
		if ($f['insurance'] != '') echo '<div class="edost_compact_hide edost_supercompact_hide edost_format_info"><span class="edost_insurance">'.$f['insurance'].'</span></div>';
		...
		foreach ($f['tariff'] as $k => $v) {
			...
			$id = 'ID_DELIVERY_'.$v['html_id'] . ($compact != '' && $v['html_id'] == 'edost_office' && $k != 0 ? '_2' : '');
			$value = $v['html_value'];
			$office_map = (isset($v['office_map']) ? $v['office_map'] : '');
			$onclick = 'submitForm('.($office_map == 'get' ? "'office', '".$v['office_mode']."'" : "'update'").')';
			$onclick_get = "window.edost_window.submit('".$id."'".($office_map == 'get' ? ", '".$v['office_mode']."'" : '').")";
			$price_long = (isset($v['price_long']) ? $v['price_long'] : '');
			$codplus = (($compact == '' || $priority == 'B') && isset($v['codplus_formatted']) && !$cod_tariff ? true : false);
			$office_detailed = (!empty($v['office_detailed']) ? '<div class="edost_button_detailed" onclick="edost_office.info(0, \''.$v['office_detailed'].'\')">'.$v['office_link'].'</div>' : false);
			$insurance = ($v['insurance'] != '' && (!$cod_tariff || empty($v['cod_tariff'])) ? true : false);
...
<?					if (!empty($v['compact_link']) && ($office_map != '' || $f_key != 'office')) { ?>
					<div class="edost_change_button edost_window_hide edost_supercompact_hide edost_order_hide edost_compact_cod_hide edost_compact_tariff_cod_hide edost_button_big2<?=($office_map == 'get' ? ' edost_button_big_red' : '')?>" style="width: auto; max-width: 200px; margin: 5px 0 0 0;" onclick="<?=(in_array($office_map, array('get', 'change')) ? "edost_office.window('".$v['office_mode']."', false)" : "edost_window.set('".$f_key."', 'head=".$v['compact_head']."')")?>">
						<span><?=$v['compact_link']?></span>
					</div>
<?					} ?>
<?					if (!empty($v['compact_link_cod']) && ($office_map != '' || $f_key != 'office')) { ?>
					<div class="edost_change_button edost_window_hide edost_supercompact_hide edost_order_hide edost_compact_nocod_hide edost_compact_tariff_nocod_hide edost_button_big2<?=($office_map == 'get' ? ' edost_button_big_red' : '')?>" style="width: auto; max-width: 200px; margin: 5px 0 0 0;" onclick="<?=(in_array($office_map, array('get', 'change')) ? "edost_office.window('".$v['office_mode']."', false)" : "edost_window.set('".$f_key."', 'head=".$v['compact_head_cod']."')")?>">
						<span><?=$v['compact_link_cod']?></span>
					</div>
<?					} ?>
<?					if (!empty($v['compact_link']) && $office_map == 'get') { ?>
					<div class="edost_change_button edost_window_hide edost_compact_hide edost_button_big2<?=($office_map == 'get' ? ' edost_button_big_red' : '')?>" style="<?=($office_map != 'get' ? 'width: auto; max-width: 200px;' : '')?> margin: 5px 0 0 0;" onclick="<?=(in_array($office_map, array('get', 'change')) ? "edost_office.window('".$v['office_mode']."', false)" : "edost_window.set('".$f_key."', 'head=".$v['compact_head']."')")?>">
						<span><?=$v['compact_link']?></span>
					</div>
<?					} ?>
...
<?					if ($office_map == 'get' && $compact == '') { ?>
					<br><span class="edost_format_link_big<?=(!empty($v['office_link2']) ? ' edost_resize_day' : '')?> edost_compact_hide edost_supercompact_hide" onclick="edost_office.window('<?=($map_inside ? 'all' : $v['office_mode'])?>');"><?=$v['office_link']?></span>
<?					if (!empty($v['office_link2'])) { ?>
					<span class="edost_format_link_big edost_resize_day2 edost_compact_hide edost_supercompact_hide" onclick="edost_office.window('<?=($map_inside ? 'all' : $v['office_mode'])?>');"><?=$v['office_link2']?></span>
<?					} ?>
<?					} ?>
				</td>
...
<?				if ($compact == '') $s = 'width="'.($price_width + ($price_long !== '' ? 30 : 0)).'" align="right"';
				else $s = 'width="105" style="text-align: center; vertical-align: center;"'; ?>
...
<?				if (!empty($v['compact_link'])) { ?>
					<div class="edost_button_big2<?=($office_map == 'get' && !empty($v['checked']) ? ' edost_button_big_red' : '')?>" onclick="<?=(in_array($office_map, array('get', 'change')) ? "edost_office.window('".$v['office_mode']."')" : "edost_window.set('".$f_key."', 'head=".$v['compact_head']."')")?>">
...
<?					if ($office_map == 'change') { ?>
					<br><span class="edost_format_link<?=(empty($v['supercompact']) ? ' edost_compact_hide edost_supercompact_hide' : '')?>" onclick="edost_office.window('<?=($map_inside ? 'all' : $v['office_mode'])?>');"><?=$v['office_link']?></span>
...
<?					if (!empty($v['day'])) { ?>
					<div><label for="<?=$id?>" style="margin: 0;"><span class="edost_format_price edost_day"><?=$v['day']?></span></label></div>
<?					} ?>
					<div>
<?					if (!in_array($v['html_id'], $tariff_price_hide)) { ?>
						<label for="<?=$id?>" class="edost_compact_cod_hide edost_compact_tariff_cod_hide">
...
<?					if ($codplus) { ?>
					<div class="edost_payment edost_compact_hide edost_supercompact_hide edost_compact_tariff_cod_hide edost_resize_cod2"><?=GetMessage('SOA_TEMPL_COD_TARIFF').($office_map != 'get' && $v['codplus_formatted'] != 0 ? ' <span class="edost_bracket">(</span><span class="edost_codplus">+ '.$v['codplus_formatted'].'</span><span class="edost_bracket">)</span>' : '')?></div>
<?					} ?>
...
<?					} ?>
					...
					<div class="edost_supercompact_hide edost_change_button edost_button_big2<?=($office_map == 'get' && !empty($v['checked']) ? ' edost_button_big_red' : '')?>" onclick="<?=(in_array($office_map, array('get', 'change')) ? "edost_office.window('".$v['office_mode']."')" : "edost_window.set('".$f_key."', 'head=".$v['compact_head']."')")?>">
						<span class="edost_window_hide"><?=$v['compact_link']?></span>
					</div>
				</td>
			</tr>
...
<?	} else if (!empty($data['format']['map_json'])) { ?>
	<input id="edost_office_data" autocomplete="off" value='{"ico_path": "<?=$ico_path?>", "yandex_api_key": "<?=(!empty($arParams['YANDEX_API_KEY']) ? $arParams['YANDEX_API_KEY'] : '')?>", <?=$data['format']['map_json']?>}' type="hidden">
<?	} ?>
