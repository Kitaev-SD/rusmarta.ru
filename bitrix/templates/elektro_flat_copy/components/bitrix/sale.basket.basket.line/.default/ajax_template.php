...
<div class="oformit_cont">
	<?if(!CSite::InDir($arParams["PATH_TO_BASKET"]) && !CSite::InDir($arParams["PATH_TO_ORDER"]) && IntVal($arResult["QUANTITY"]) > 0):?>
		<form action="<?=$arParams['PATH_TO_BASKET']?>" method="post">
			<button class="btn_buy popdef oformit" name="oformit" class="btn_buy popdef oformit" value="<?=GetMessage('BASKET_LINE_CHECKOUT')?>"><?=GetMessage("BASKET_LINE_CHECKOUT")?></button>
		</form>
	<?else:?>
