...
<div class="cart-items" id="id-cart-list">
    ...
    <div  id="min_price_message"  class="alertMsg info disN">
		  <i class="fa fa-info"></i>
		   <span class="text"><?=GetMessage('MIN_PRICE_VALUE')?><?=$arSetting["ORDER_MIN_PRICE"]["VALUE"]?><?=$arResult["allSum_CURRENCY"]?></span>
	</div> 
	<?if(count($arResult["ITEMS"]["AnDelCanBuy"]) > 0):?>
		...
		<div class="equipment" id="cart_equipment">
			...
			<div class="tbody">
				...
				foreach($arResult["ITEMS"]["AnDelCanBuy"] as $arBasketItems) {
                    ...
                    $db_props = CIBlockElement::GetProperty(16, $arBasketItems['PRODUCT_ID'], ["SORT" => "ASC"], ["CODE"=>"ym_link"]);
					...
					<div class="tr" id="<?=$arBasketItems['ID']?>">
						<div class="tr_into">							
							<div class="cart-item-image" <?if(isset($YM) && !empty($YM)):?>data-ym="<?=$YM?>"<?endif;?>>
							...
							<?if(in_array("DELAY", $arParams["COLUMNS_LIST"])):?>
								<div class="cart-item-actions">								
									<div class="delay">
										<a onclick="yaCounter22745254.reachGoal('aside'); return true;" class="setaside" href="<?=str_replace("#ID#", $arBasketItems["ID"], $arUrlTempl["delay"])?>" title="<?=GetMessage("SALE_OTLOG")?>"><i class="fa fa-heart-o"></i></a>
									</div>
					...
					<?$i++;
                    $SUMM += $arBasketItems['PRICE'];
                    $arIDS[$arBasketItems['PRODUCT_ID']] = $arBasketItems['PRODUCT_ID'];
				}?>
		...
		<div class="w100p">			
			...
			<div id="btn_buy_basket" class="cart-buttons">
				<?if(in_array("BUTTON_BOC", $arSetting["GENERAL_SETTINGS"]["VALUE"])):?>
					<button onclick="ym(22745254,'reachGoal','byoneclick'); ga('send', 'event', 'byoneclick', 'click');" type="button" id="boc_anch_cart" class="btn_buy boc_anch_cart" name="boc_anch_cart" value="<?=GetMessage('SALE_BOC')?>"><?=GetMessage('SALE_BOC')?></button>
				<?endif;?>
				<button type="button" id="btn_basket_order"  name="BasketOrder" class="btn_buy popdef bt3" value="<?=GetMessage('SALE_ORDER')?>" onclick="checkOut(); ym(22745254,'reachGoal','order'); ga('send', 'event', 'order', 'click');"><?=GetMessage("SALE_ORDER")?></button>
			</div>
