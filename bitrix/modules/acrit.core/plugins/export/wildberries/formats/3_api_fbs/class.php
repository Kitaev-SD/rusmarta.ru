				...
				foreach($arItem['nomenclatures'][0]['variations'] as $key => &$arVariation){
					...
					foreach($arVariation as $field => $value){
						if(preg_match('#^stock_(\d+)$#', $field, $arMatch)){
							$price = null;
							if(is_array($arVariation['addin'])){
								foreach($arVariation['addin'] as $addin){
									if($addin['type'] == static::getMessage('CUSTOM_ATTR_PRICE')){
										$price = $addin['params'][0]['count'];
										break;
									}
								}
							}
							if($chrtId){
								$arDataMore['stocks'][] = [
									...
									'storeId' => intVal($arMatch[1]),
								];
							}
							else{
								$arDataMore['stocks_empty'][] = [
									'chrtId' => 'EMPTY',
									'price' => $price,
									'quantity' => intVal($value),
									'storeId' => intVal($arMatch[1]),
	...
	public function stepExportStocks($intProfileID, $arData){
		...
		foreach($arExportItems as $arItem){
			if($arDataMore = unserialize($arItem['DATA_MORE'])){
				if($arDataMore['nmId']){
					$arStocks[] = [
						'nmId' => $arDataMore['nmId'],
						'stocks' => $arDataMore['stocks'],
					];
				}
			}
		}
		# Tmp
		# $arStocks[] = ['nmId' => 123, 'stocks' => ['10240' => 123]];
		...
		if(!empty($arStocks)){
			...
			$bSuccess = !!$arJsonResponse['success'];
