...
$MESS[$strName.'stock'] = 'Остаток на складе (<b>тестовый режим!</b>)';
	$MESS[$strHint.'stock'] = 'Общий остаток товара на складе.<br/><br/>
	<b>Внимание! функционал выгрузки остатков работает пока в тестовом режиме.</b> Остаток обновляется только для товаров, у которых этап обработки прошёл статус <b>processed</b>. До этого попытки выгрузки остатков будут завершаться ошибкой «[NOT_FOUND] Product not found». Т.е. первичная выгрузка в любом случае не обновит остатки.';
