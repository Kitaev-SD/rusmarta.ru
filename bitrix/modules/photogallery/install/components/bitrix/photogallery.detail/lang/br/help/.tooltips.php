<?
$MESS["CACHE_TIME_TIP"] = "Especifique aqui o período durante o qual o cache é válido. 
";
$MESS["CACHE_TYPE_TIP"] = "<i>Auto</i>: o cache é válido durante o período pré-definido nas configurações de cache;<br /><i>Cache</i>: sempre utilizar cache para o período especificado no próximo campo;<br /><i>Não utilizar cache</i>: nenhum cache é executado.";
$MESS["DATE_TIME_FORMAT_TIP"] = "Especifica o formato de exibição da data.";
$MESS["DETAIL_EDIT_URL_TIP"] = "Especifica o endereço da página de parâmetros da foto.";
$MESS["DETAIL_URL_TIP"] = "Especifica o endereço da página de visualização da foto completa.";
$MESS["DISPLAY_PANEL_TIP"] = "Se selecionado, os botões do editor irão ser exibidos no modo de edição do Site na barra de ferramentas do Painel de Controle e na área da caixa de ferramentas do componente.";
$MESS["ELEMENT_CODE_TIP"] = "Especifique aqui o código mnemônico do elemento (foto).";
$MESS["ELEMENT_ID_TIP"] = "Este campo contém a expressão que avalia a ID do elemento (foto).
";
$MESS["ELEMENT_SORT_FIELD_TIP"] = "Especifica o campo que será utilizado para classificar as fotos.";
$MESS["ELEMENT_SORT_ORDER_TIP"] = "Especifica a ordem em que as fotos serão classificadas.";
$MESS["IBLOCK_ID_TIP"] = "Especifique aqui o bloco de informações que irá armazenar as fotos. Alternativamente, você pode selecionar <b>(outro)-></b> e especificar a ID do bloco de informações no campo ao lado.
";
$MESS["IBLOCK_TYPE_TIP"] = "Selecione aqui um dos tipos existentes de blocos de informação. Clique em <b><i>OK</i></b> para carregar os blocos de informação do tipo selecionado.";
$MESS["SECTION_CODE_TIP"] = "Especifique aqui o código mnemônico da seção (álbum).";
$MESS["SECTION_ID_TIP"] = "
Este campo contém a expressão que avalia a ID da seção (álbum).
";
$MESS["SECTION_URL_TIP"] = "Especifica o endereço da página de visualização do álbum.";
$MESS["SET_TITLE_TIP"] = "Se selecionado, o título da página será ajustado como o nome do álbum atual.";
$MESS["UPLOAD_URL_TIP"] = "Especifica o endereço da página de upload de imagens.";
?>