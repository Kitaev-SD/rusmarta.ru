				...
				if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"])
				{
					...
					if(!$arErrors[$optionsSiteID])
					{
						if($arTab["ITEMS"])
						{
							...
							$LANDINGS_SEARCH_IBLOCK_ID = Cache::$arIBlocks[$optionsSiteID]['aspro_next_catalog']['aspro_next_search'][0];

							$arFiles = $sitemapLandingsSearch = array();
							foreach(glob($arTab["SITE_DIR_FORMAT"].$siteMapNameTmp.'*.xml', 0) as $dir){
								$dir = str_replace($arTab["SITE_DIR_FORMAT"], '', basename($dir));
								$arFiles[] = $dir;

								if($LANDINGS_SEARCH_IBLOCK_ID && strpos($dir, $LANDINGS_SEARCH_IBLOCK_ID) !== false){
									$sitemapLandingsSearch[] = $dir;
								}
							}

							// get content of sitemap of landings in search
							if($sitemapLandingsSearch){
								$arLandings = $arLandingsIDs = array();

								foreach($sitemapLandingsSearch as $lsfile){
									if(file_exists($arTab['SITE_DIR_FORMAT'].$lsfile)){
										$content = @file_get_contents($arTab['SITE_DIR_FORMAT'].$lsfile);
										if($content && preg_match_all('/<url>\s*<loc>(([^<]*)=(\d*))<\/loc>\s*<lastmod>([^<]*)<\/lastmod>\s*<\/url>/i', $content, $arLandingsMatches)){
												$arLandingsIDs = array_merge($arLandingsIDs, $arLandingsMatches[3]);
										}
									}
								}
								...
								foreach($arFiles as $xmlfile)
								{
									...
									if($siteMapNameTmp)
									{
										foreach($arTab["ITEMS"] as $arItem)
										{
											...
											if($arLandings && in_array($xmlfile, $sitemapLandingsSearch)){
												...
												if($content = @file_get_contents($regionSitemapLandingSearch)){
													if(preg_match_all('/<url>\s*<loc>(([^<]*)=(\d*))<\/loc>\s*<lastmod>([^<]*)<\/lastmod>\s*<\/url>/i', $content, $arLandingsMatches)){
