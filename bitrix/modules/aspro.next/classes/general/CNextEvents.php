		...
		if(!defined('ADMIN_SECTION') && !defined('WIZARD_SITE_ID') || $bCompSaleOrderAjaxPost)
		{
			...
			if($GLOBALS['_USE_LAZY_LOAD_']){
				...
				if(isset($tmpContent)){
					...
					$content = preg_replace('/<img lazyload (?![^>]*\bclass\s*=\s*[\'\"]\b)/i'.BX_UTF_PCRE_MODIFIER, '${0}class="lazyload" ', $content);
				}
				else{
					// preg error
				}
	...
	static function OnCatalogDeliveryComponentInitUserResult(&$arResult, &$arParams, $request){
		...
		if(!$arResult['LOCATION']){
			...
			if($arRegion && $arRegion['LOCATION']){
				if($arResult['LOCATION'] = CAsproCatalogDeliveryNext::getLocationByCode(CSaleLocation::getLocationCODEbyID($arRegion['LOCATION']), LANGUAGE_ID)){
					$arResult['LOCATION_SOURCE'] = 'region';
				}
			}
		}
	}
