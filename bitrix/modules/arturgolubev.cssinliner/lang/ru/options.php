<?
$MESS["ARTURGOLUBEV_CSSINLINER_ERROS_SETTING_TITLE"] = "Рекомендации:<br>";
$MESS["ARTURGOLUBEV_CSSINLINER_MAINMODULE_OPTIMIZE_CSS"] = "В настройках <a href=\"/bitrix/admin/settings.php?lang=ru&mid=main\" target=\"_blank\">главного модуля</a> отключите параметр \"Объединять CSS файлы\"";


$MESS["ARTURGOLUBEV_CSSINLINER_CSS_INLINE_TAB"] = "Настройки обработки";


$MESS["ARTURGOLUBEV_CSSINLINER_ENABLE"] = "Модуль отключен:";
$MESS["ARTURGOLUBEV_CSSINLINER_DISABLED_SITE"] = "Отключить для сайта";


$MESS["ARTURGOLUBEV_CSSINLINER_WORKING_WITH_STYLE"] = "Настройка Инлайн подключения стилей (Inline CSS)";
// $MESS["ARTURGOLUBEV_CSSINLINER_INLINE_MAX_WEIGHT"] = "<span data-hint='Позволяет установить максимальный вес файла, подключаемого по технологии Inline'></span>Не обрабатывать css-файлы весом более (кб):";
$MESS["ARTURGOLUBEV_CSSINLINER_INLINE_MAX_WEIGHT"] = "<span data-hint='Позволяет установить максимальный вес файла, подключаемого по технологии Inline'></span>Максимальный вес файлов подключамых в inline-виде (кб):";
$MESS["ARTURGOLUBEV_CSSINLINER_USE_COMPRESS"] = "<span data-hint='Оптимизация стилей происходит за счёт удаления символов табуляции, переносов, комментариев и т.п.'></span>Оптимизировать обрабатываемый css:";
$MESS["ARTURGOLUBEV_CSSINLINER_GOOGLE_FONTS_INLINE"] = "Подключать Google Fonts в inline-виде:";
$MESS["ARTURGOLUBEV_CSSINLINER_OUTER_STYLE_INLINE"] = "Подключать внешние стили в inline-виде:";
$MESS["ARTURGOLUBEV_CSSINLINER_DELETE_OPEN_SANS"] = "<span data-hint='Опцию следует использовать если шрифт Open Sans не используется'></span>Удалить подключаемый ядром битрикса Open Sans (BETA):";
$MESS["ARTURGOLUBEV_CSSINLINER_USE_FONT_DISPLAY"] = "<span data-hint='Если у подключаемых в css шрифтов не задан параметр font-display, устанавливает параметр font-display: swap'></span>Использовать font-display для шрифтов (BETA):";
$MESS["ARTURGOLUBEV_CSSINLINER_EXCEPTIONS"] = "<span data-hint='В данной опции можно указать стили, которые не требуется обрабатывать. Формат /bitrix/js/tests.banner/style.css'></span>Стили исключения:<br>(каждый с новой строки)";
$MESS["ARTURGOLUBEV_CSSINLINER_ADMIN_DEBUG"] = "Включить отладку";


$MESS["ARTURGOLUBEV_CSSINLINER_WORKING_PRECONNECT"] = "Настройка Предсоедиения (Preconnect)";
$MESS["ARTURGOLUBEV_CSSINLINER_PRECONNECT"] = "<span data-hint='Доменные имена указывайте в формате https://yandex.ru'></span>Доменные имена к которым осуществлять предсоедиенение:<br>(каждое с новой строки)";
$MESS["ARTURGOLUBEV_CSSINLINER_PRECONNECT_NOTE"] = "Preconnect рекомендуется использовать когда у Вас на сайте используются скрипты/стили/шрифты с внешних сервисов. Например: Яндекс.Метрика, CDN Jquery, Google Шрифты. В этом случае полезно делать предсоединение к доменам используемых сервисов, указывая в настройке соотвествующие домены: https://mc.yandex.ru , https://ajax.aspnetcdn.com , https://fonts.googleapis.com";

$MESS["ARTURGOLUBEV_CSSINLINER_WORKING_PRELOADING"] = "Настройка Предзагрузки (Preloading)";
$MESS["ARTURGOLUBEV_CSSINLINER_PRELOADING"] = "<span data-hint='Ресурсы указывайте в формате /bitrix/js/tests.banner/scripts.js'></span>Ресурсы, которые нужно предзагружать:<br>(каждое с новой строки)";
$MESS["ARTURGOLUBEV_CSSINLINER_PRELOADING_NOTE"] = "Preloading рекомендуется использовать Только при необходимости - если Google PageSpeed рекомендует \"Настройте предварительную загрузку ключевых запросов\" к конкретным скриптам указанным в рекомендации.";


$MESS["ARTURGOLUBEV_CSSINLINER_ALLOW_URL_FOPEN_NOT_FOUND"] = "Отключен параметр PHP allow_url_fopen";
$MESS["ARTURGOLUBEV_CSSINLINER_ALLOW_URL_FOPEN_NOT_FOUND_TEXT"] = "Для работы модуля с внешними стилями и Google Fonts требуется установить параметр PHP allow_url_fopen = On. Для настройки рекоммендуется обратиться в поддержку хостинга или к администратору";



$MESS["ARTURGOLUBEV_CSSINLINER_CLEAR_CACHE"] = 'Настройки модуля изменены. Очистите <a target="_blank" href="/bitrix/admin/cache.php?lang=ru">Все страницы HTML кеша</a> и закройте это уведомление';
$MESS["ARTURGOLUBEV_CSSINLINER_NOTE_MAIN_JS"] = "Для максимальной эффективности необходимо установить параметр <b>\"Переместить весь Javascript в конец страницы\"</b> <a href=\"/bitrix/admin/settings.php?lang=ru&mid=main\" target=\"_blank\">в настройках главного модуля</a> ";
$MESS["ARTURGOLUBEV_CSSINLINER_MAIN_NOTE"] = "<b>Внимание:</b> Для удобства и безопасности замена в режиме администратора решение неактивно.";
$MESS["ARTURGOLUBEV_CSSINLINER_DEMO_IS_EXPIRED"] = "Демонстрационный период работы решения закончился. Для дальнейшего использования необходимо приобрести полую версию решения в <a href=\"http://marketplace.1c-bitrix.ru/solutions/arturgolubev.cssinliner/\" target=\"_blank\">marketplace.1c-bitrix.ru</a>";


$MESS["ARTURGOLUBEV_CSSINLINER_USE_INLINE"] = "Подключить таблицы стилей как inline код";



/* help tab */
$MESS["ARTURGOLUBEV_CSSINLINER_HELP_TAB_NAME"] = "Информация";
$MESS["ARTURGOLUBEV_CSSINLINER_HELP_TAB_TITLE"] = "Полезная информация";

$MESS["ARTURGOLUBEV_CSSINLINER_CARD_TEXT"] = "Карточка решения на Marketplace -";
$MESS["ARTURGOLUBEV_CSSINLINER_CARD_TEXT_VALUE"] = "<a href='https://marketplace.1c-bitrix.ru/solutions/arturgolubev.cssinliner/#tab-about-link' target='_blank'>ссылка</a>";

$MESS["ARTURGOLUBEV_CSSINLINER_INSTALL_TEXT"] = "Информация по установке -";
$MESS["ARTURGOLUBEV_CSSINLINER_INSTALL_TEXT_VALUE"] = "<a href='https://marketplace.1c-bitrix.ru/solutions/arturgolubev.cssinliner/#tab-install-link' target='_blank'>ссылка</a>";

$MESS["ARTURGOLUBEV_CSSINLINER_INSTALL_VIDEO_TEXT"] = "Видео-инструкция -";
$MESS["ARTURGOLUBEV_CSSINLINER_INSTALL_VIDEO_TEXT_VALUE"] = "<a href='http://arturgolubev.ru/learning/course/index.php?COURSE_ID=7&INDEX=Y' target='_blank'>ссылка</a>";

$MESS["ARTURGOLUBEV_CSSINLINER_FAQ_TEXT"] = "Часто задаваемые вопросы по данному модулю -";
$MESS["ARTURGOLUBEV_CSSINLINER_FAQ_TEXT_VALUE"] = "<a href='http://arturgolubev.ru/learning/course/index.php?COURSE_ID=7&INDEX=Y' target='_blank'>ссылка</a>";

$MESS["ARTURGOLUBEV_CSSINLINER_FAQ_MAIN_TEXT"] = "Вопросы по покупке, оплате, активации модуля и т.п. -";
$MESS["ARTURGOLUBEV_CSSINLINER_FAQ_MAIN_TEXT_VALUE"] = "<a href='http://arturgolubev.ru/learning/course/?COURSE_ID=1&INDEX=Y' target='_blank'>ссылка</a>";
?>