...
if ( $RIGHT >= "R" ) :
    ...
    <form method="post"
		...
		foreach ( $arAllOptions as $arOption ):
            ...
            <tr>
                <td width="40%" nowrap <?
				if ( $type[0] == "textarea" || $type[0] == "multiple_select" )
                ...
                <td width="60%">
					...
					if ( $type[0] == "checkbox" ): ?>
						...
						if ( $disabled ) echo ' disabled="disabled"' ?>
								...
								echo htmlspecialcharsbx( $arOption[0] ) ?>"
                                value="Y" <?
						if ( $val == "Y" ) echo ' checked="checked"'; else echo ''; ?>>
					<?
                    elseif ( $type[0] == "text" ): ?>
                        <input
                                type="text"
                    ...
                    elseif ( $type[0] == "multiple_select" ): ?>
                        <select name="<?
						echo htmlspecialcharsbx( $arOption[0] ) ?>[]" multiple size="4"><?
							foreach ( $arOption[5] as &$oneVal ) {
								?>
                                <option value="<?
								echo htmlspecialcharsbx( $oneVal['value'] ); ?>"<?
								echo( isset( $val[ $oneVal['value'] ] ) ? ' selected' : '' ); ?>><?
								echo htmlspecialcharsex( $oneVal['name'] ); ?></option><?
							}
							if ( isset( $oneVal ) )
								unset( $oneVal );
							unset( $val );
