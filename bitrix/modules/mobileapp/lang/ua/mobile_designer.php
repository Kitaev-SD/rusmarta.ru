<?
$MESS["MOBILEAPP_ANDROID"] = "OS Android";
$MESS["MOBILEAPP_IOS"] = "iOS";
$MESS["MOBILEAPP_WP"] = "Windows Phone";
$MESS["MOBILEAPP_GLOBAL"] = "Загальна";
$MESS["MOBILEAPP_CONFIGNAME_ANDROID"] = "Конфігурація для OS Android";
$MESS["MOBILEAPP_CONFIGNAME_GLOBAL"] = "Загальна конфігурація";
$MESS["MOBILEAPP_CONFIGNAME_IOS"] = "Конфігурація для iOS";
$MESS["MOBILEAPP_CONFIGNAME_WP"] = "Конфігурація для Windows Phone";
$MESS["MOBILEAPP_ADD_IMAGE"] = "Додати зображення";
$MESS["MOBILEAPP_SELECT_IMAGE"] = "Вибрати картинку";
$MESS["MOBILEAPP_APP_PLATFORM"] = "Додати платформу";
$MESS["MOBILEAPP_SAVE"] = "Зберегти";
$MESS["MOBILEAPP_CREATE"] = "Створити";
$MESS["MOBILEAPP_CLOSE"] = "Закрити";
$MESS["MOBILEAPP_REMOVE"] = "Видалити";
$MESS["MOBILEAPP_BACK"] = "Назад";
$MESS["MOBILEAPP_TITLE"] = "Тема";
$MESS["MOBILEAPP_NO_APPS"] = "У вас немає жодного додатки";
$MESS["MOBILEAPP_CREATE_APP"] = "Створити застосунок";
$MESS["MOBILEAPP_APP_NAME"] = "Назва";
$MESS["MOBILEAPP_LABEL_CODE"] = "Код додатки (латиницею)";
$MESS["MOBILEAPP_LABEL_APP_FOLDER"] = "Папка додатки";
$MESS["MOBILEAPP_PREVIEW_MAIN"] = "Основний";
$MESS["MOBILEAPP_PREVIEW_LOAD"] = "Завантаження";
$MESS["MOBILEAPP_PREVIEW_LISTS"] = "Списки";
$MESS["MOBILEAPP_CREATE_NEW_TEMPLATE"] = "Створити новий шаблон";
$MESS["MOBILEAPP_WITHOUT_TEMPLATE"] = "Не створювати шаблон";
$MESS["MOBILEAPP_NEW_TEMPLATE_NAME"] = "Назва шаблону";
$MESS["MOBILEAPP_SELECT_TEMPLATE"] = "Вибрати існуючий";
$MESS["MOBILEAPP_APP_TEMPLATE"] = "Шаблон додатки";
$MESS["MOBILEAPP_PREVIEW_HINT"] = "Натисніть на елемент програми, щоб налаштувати його параметри";
$MESS["MOBILEAPP_APP_SAVE_CONFIG_WAIT"] = "Збереження конфігурації";
$MESS["MOBILEAPP_APP_REMOVE_CONFIG_WAIT"] = "Видалення конфігурації";
$MESS["MOBILEAPP_APP_CREATE_CONFIG_WAIT"] = "Створення конфігурації";
$MESS["MOBILEAPP_APP_IS_ALREADY_EXISTS"] = "Застосунок із заданим символьним кодом вже існує!";
$MESS["MOBILEAPP_ADD_FILE"] = "Додати файл";
$MESS["MOBILEAPP_EMPTY_VALUE"] = "Не вибрано";
$MESS["MOBILEAPP_CONNECT_TO_APP"] = "Підключитися";
$MESS["MOBILEAPP_CONNECT_BUTTON_CLOSE"] = "Закрити";
$MESS["MOBILEAPP_CONNECT_TO_APP_TITLE"] = "Підключитися до застосунка";
$MESS["MOBILEAPP_CONNECT_INSTRUCTION"] = "Для підключення використовуйте застосунок для розробників BitrixMobile з <a href='https://play.google.com/store/apps/details?id=com.bitrix.market.developers&hl=ru' target='_blank'>Google Play</a> або <a href='https://itunes.apple.com/ru/app/bitrixmobile/id623842918?mt=8' target='_blank'>AppStore</a>.</br> Запустіть програму і введіть наступну адресу:";
$MESS["MOBILEAPP_CONNECT_INSTRUCTION_QR"] = "або відскануйте qr-код:";
$MESS["MOBILEAPP_CREATE_APP_TEMPLATE_OPTIONS_TITLE"] = "Налаштування публічної частини";
$MESS["MOBILEAPP_CREATE_APP_PUBLIC_OPTIONS_TITLE"] = "Налаштування публічної частини";
$MESS["MOBILEAPP_CREATE_APP_PUBLIC_TEMPLATE_SIMPLE"] = "Звичайний застосунок";
$MESS["MOBILEAPP_CREATE_APP_PUBLIC_TEMPLATE_API"] = "Демо-застосунок";
$MESS["MOBILEAPP_CREATE_OFFLINE"] = "Додати шаблон і налаштування офлайн застосунка";
$MESS["MOBILEAPP_SELECT_FILE_NAME"] = "виберіть файл";
$MESS["MOBILEAPP_SELECT_FILE"] = "виберіть файл";
$MESS["MOBILEAPP_REMOVE_CONFIG_ASK"] = "Всі збережені налаштування конфігурації будуть загублені.
Видалити конфігурацію?";
$MESS["MOBILEAPP_REMOVE_APP_ASK"] = "Всі збережені налаштування застосунка будуть загублені.
Файли, створені застосунокм не будуть видалені.
Видалити застосунок з конструктора?";
?>